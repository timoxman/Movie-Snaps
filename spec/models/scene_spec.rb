...
describe Scene do
  before(:each) do
  	set_omniauth
  	Location.create(id:1,description:"This is a location")
  ...
  it 'can be created when movie and location' do
  ...
  it 'cannot be created without a movie' do
  	scene = Scene.create(location_id:1,image_file_name:"somepic.jpg")
  	expect(Scene.count).to eq 0
  end

  it 'cannot be created unless there is a location' do
  	scene = Scene.create(movie_id:1,image_file_name:"somepic.jpg")
  	expect(Scene.count).to eq 0
  	Scene.create(movie_id:1,location_id:1,image_file_name:"somepic.jpg")
  	expect(Scene.count).to eq 1
  end
